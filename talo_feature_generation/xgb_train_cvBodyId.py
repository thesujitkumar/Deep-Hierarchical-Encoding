...
def build_data():
    ...
    feature_left = features.pop(1)
    features.pop(1)
    
    print(' len of features:', len(features))
    for i, item in enumerate(features):
	print(i, item.shape, item.ndim)

    #data_x=scipy.hstack(features)
    #data_x = np.concatenate(features, 0)
    #data_x = np.array(features)
    #data_x = np.hstack(features[:2])
    data_x = np.hstack(features)
    #data_x = np.hstack((feature_left, data_x))
    print data_x[0,:]
    print 'data_x.shape'
    print data_x.shape
    print 'data_y.shape'
...
def build_test_data():
    ...
    data = pd.read_csv("FNC_Bin_Test.csv")
...
def train():
    ...
    w = np.array([1 if y == 1 else 1 for y in data_y])
    #print 'w:'
    #print w
    #print np.mean(w)
    ...
    #bst = xgb.train(params_xgb, 
                    #dtrain,
                    #n_iters)
    clf = GradientBoostingClassifier(n_estimators=50, random_state=14128, verbose=True)
    clf.fit(data_x, data_y)
    #dtest = xgb.DMatrix(test_x)
    pred_y=clf.predict(test_x)
    df=pd.DataFrame(pred_y)
    df.to_csv("FNC_Bin_Talo.csv",index=False)
    exit(1)
