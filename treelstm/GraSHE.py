...
class DocLSTM(nn.Module):
    def __init__(self, vocab_size, in_dim, mem_dim, sparsity, freeze, max_num_para, max_num_sent):
        ...
        self.selective_gate= nn.Linear(self.mem_dim*4, 2*self.mem_dim)
    ...
    def forward(self, body):
        ...
        for p_id in body: # loop will b number of paragraphs
            ...
            # print("shape sentence encoding enput",sentences_encoding.shape )
        ...
        h_t_prime_list_tensor = torch.cat(h_t_prime_list, 0).view(self.max_num_para, 2*self.mem_dim)

        #print("shape of h_t list ",h_t_prime_list_tensor.shape ) torch.t
        h_t_prime_list_tensor_transpose = torch.t( h_t_prime_list_tensor)#.transpose(0,1)
        #h_t_prime_list_tensor_transpose = h_t_prime_list_tensor.transpose(0,1)
        #print(" transpose shape of h_t list ",h_t_prime_list_tensor_transpose.shape )
        sim = torch.matmul(h_t_prime_list_tensor,h_t_prime_list_tensor_transpose)
        #print("similairty matrix dimension", sim.shape)
        sim_entropy = self.calculate_entropy_matrix(sim)
        sim_entropy= torch.sigmoid(sim_entropy)
        # print("sim entropy after sigmoid",sim_entropy)
        sim_entropy= torch.subtract(torch.tensor(1),sim_entropy)
        # print("sim entropy after subtract",sim_entropy)
        #print("sim entripy after subtracting from 1",sim_entropy)

        sim_entropy_normalized = sim_entropy/sim_entropy.sum()
        # print("normalized entropy",sim_entropy_normalized)
        #print(" the shape of entropy over similairty",sim_entropy.shape)
        #print("similarity entropy value are",sim_entropy)
        h_body_prime = torch.matmul(sim_entropy_normalized.view(1, self.max_num_para), h_t_prime_list_tensor)
        #print("final weighted vector shape",h_body_prime.shape)
        #print("final weighted vector ",h_body_prime)
        ...
        # print("final representations of body",h_body_prime.shape)
        # print("final representations of headline",rhidden.shape)
        del h_t_prime_list
        del h_body_list
        del p_hidden_list
        #del body
    ...
    def calculate_entropy_matrix(self, X):
        #print('X:', X)
        entropy_vector=[] #torch.zeros(self.max_num_para,requires_grad = True)
        X =  (X-X.min()) / X.sum(dim=1, keepdim=True)
        #print(' X after norm : ', X)
        entropy_vector =  Categorical(probs = X).entropy()
        #
        #
        ...
        #         if i == j:
        #             continue
        #         cur_value +=(-X[i][j]*torch.log(X[i][j]))
        #     entropy_vector.append(cur_value)
        # entropy_vector= torch.tensor(entropy_vector, requires_grad=True)
        # print('entropy_vector :', entropy_vector)
...
class SimilarityTreeLSTM(nn.Module):
    ...
    def forward(self, body, feature_vec=None):
        ...
        output = self.similarity( c_body.view(1, 2*self.hidden_dim) , c_headline,  feature_vec)
        return output
