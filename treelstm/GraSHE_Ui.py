...
class DocLSTM(nn.Module):
    def __init__(self, vocab_size, in_dim, mem_dim, sparsity, freeze, max_num_para, max_num_sent):
        ...
        self.selective_gate= nn.Linear(self.mem_dim*2,self.mem_dim)
    ...
    def forward(self, body):
        ...
        h_body_list = []
        c_t_minus_one =  torch.zeros(1, 1, self.mem_dim)
        h_t_minus_one =   torch.zeros(1, 1, self.mem_dim)
        for p_hidden in p_hidden_list:
            out_body_t, (h_body_t, c_body_t) =  self.body_LSTM(p_hidden, (h_t_minus_one, c_t_minus_one))
            # save ht_body_t
            h_body_list.append(h_body_t)
            c_t_minus_one = c_body_t
            h_t_minus_one = h_body_t
        ...
        for h_t in h_body_list:
            exp_buf = torch.cat((h_t.view(1,self.mem_dim), h_body_list[-1].view(1,self.mem_dim)),1)
        ...
        h_t_prime_list_tensor = torch.cat(h_t_prime_list, 0).view(self.max_num_para, self.mem_dim)
