...
class DocLSTM(nn.Module):
    ...
    def forward(self, body):
        ...
        for p_id in body: # loop will b number of paragraphs
            ...
            #print("shape of input to Para LSTM",sentences_encoding.shape )
            ...
            #print("shape of h_para",h_para.shape)
            p_hidden_list.append(h_para.view(1,2*self.mem_dim))
            del s_hidden_list
        ...
        del p_hidden_list
        #print("paragraph_encoding", paragraph_encoding.shape)
        out_body, (h_body, c_body)=self.body_LSTM(paragraph_encoding.contiguous().view(self.max_num_para, 1, 2*self.mem_dim )) # ccall body_LSTM function to encode the several paragraph to final represnetation
        h_body_2d= h_body.view(2,self.mem_dim)
        h_left= h_body_2d[0]
        h_right= h_body_2d[1]
        h_body= torch.cat((h_left,h_right),0)
...
class SimilarityTreeLSTM(nn.Module):
    ...
    def forward(self, body, feature_vec= None):
        ...
        output = self.similarity( c_body.view(1, self.in_dim) , c_headline,  feature_vec)
        return output
